...
const MinimalisticCarouselItem = ({ title, value, description, icon, viewMoreUrl, styles }) => {
  const { formatMessage } = useIntl()
  const Icon = getDynamicIcon(icon)
  const cardTitle = value ? `${value} ${title}` : title

  return (
    <Box sx={styles.cardRoot} className="env-impact-slide">
      <Icon />
      <Text variant="body-primary-md" sx={styles.cardTitle}>
        {cardTitle}
      </Text>
...
export default MinimalisticCarouselItem
